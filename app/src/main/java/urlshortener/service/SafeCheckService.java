...
public class SafeCheckService {
  ...
  public CompletableFuture<List<String>> safeBrowsingChecker(String url) throws IOException {
    ...
    String safeBrowsingUrl = "https://safebrowsing.googleapis.com/v4/threatMatches:find?key="
        + SECRET_GOOGLE_SAFE_BROWSING_KEY;
    urlsList.add(url);
    ...
    // Case when url is safe
    if (jsonObject.isEmpty()) {
      resultList.add("SAFE");
    } else {

      // Case when url is unsafe
      // Adds description of why it is unsafe
